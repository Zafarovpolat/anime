...
## 2. Этап 1 — Комментарии (в работе, приоритет №1)
...
| 13 | Бан за нецензурные слова в комментариях (админка) | 300 | ⏳ Не начато — в коде `admin/comments` и `admin/users` нет автодетекта мата и общего механизма бана из комментария |
| 14 | Древовидные комментарии (полноценная вложенность) | 500 | ⏳ Не начато — сейчас ответ на комментарий просто подставляет `@username, ` в поле ввода нового комментария, без привязки к родителю (см. `manga/[id]/page.tsx` и `read/page.tsx`, кнопка «Ответить») |

**Открытый вопрос по п.14**: 29.07 Пулат предложил заказчику 3 варианта реализации, решение
**не зафиксировано в чате**:
1. Без вложенности, но с кликабельной пометкой «в ответ на @Ник» — 1к / 1 день
