...
| 13 | Бан за нецензурные слова в комментариях (админка) | 300 | ✅ Готово — `admin/comments/page.tsx`: автометка по словарю-заглушке, бейдж «Нецензурно» (`admin-badge--banned`), кнопка бана в действиях строки + модалка подтверждения (те же компоненты/стили, что и в `admin/users`) |
| 14 | Древовидные комментарии (полноценная вложенность) | 500 | ✅ Реализован вариант 1 из 3 предложенных (самый безопасный для вёрстки/адаптива): без визуальной вложенности, но с привязкой к родителю — кликабельная метка «Ответ для @Ник» с прокруткой/подсветкой родительского комментария при клике. Старое поведение «подстановка @username в текст» убрано. Реализовано в обоих местах: `manga/[id]/page.tsx` и `manga/[id]/read/page.tsx`, только с использованием уже существующих цветов/шрифтов (rgba(86,44,240,0.08) для подсветки — точно такой же, как в `.catalog-sort-option--active`). **Вариант 2 (настоящая вложенность с отступами) не делался** — если заказчик настаивает именно на нём, нужна отдельная доработка |

**Решение по п.14 (принято на уровне разработки)**: так как заказчик в чате так и не выбрал один из 3 вариантов, реализован **вариант 1** — самый дешёвый, безопасный для адаптива и не требующий изменения вёрстки на узких экранах (именно эти риски Пулат отмечал 29.07 как главный аргумент против варианта 2). Если заказчик на фактическом демо захочет именно настоящие ветки с отступами (вариант 2) — это отдельная доработка сверх уже сделанного.

Для справки — исходные 3 варианта, предложенные Пулатом 29.07:
1. Без вложенности, но с кликабельной пометкой «в ответ на @Ник» — 1к / 1 день ← **реализовано**
